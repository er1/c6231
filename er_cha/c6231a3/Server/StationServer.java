...
public class StationServer implements StationInterface {
    ...
    public static void main(String[] args) {
        ...
        for (String stationName : stations) {
            ...
            station.startServerThreads(registry);
        }
    }
     ...
     * @throws RemoteException
     */
    public void exportRPC(Registry registry) {

        log.log("Attempting to publish RPC for " + this.name);

        Remote remote;
        try {
            remote = UnicastRemoteObject.exportObject(this, 9989);
            registry.rebind(this.name, remote);
        } catch (RemoteException ex) {
            log.log("RPC for " + name + "Failed");
            return;
        }

        log.log("Published RPC for " + name);

        log.log("Attempting to start...");
    ...
    public void startServerThreads(final Registry registry) {
